...
* upload only media or staticfiles files.
...
Command options are:
  ...
  --staticfiles-only         Only STATIC_ROOT files will be uploaded to S3.
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '-d', '--dir',
            dest='dir',
            help="Custom staticfiles root directory to use"
        )
        parser.add_argument(
            '--s3host',
            dest='s3host',
        ...
        parser.add_argument(
            '--staticfiles-only', dest='static_only', default='',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        if self.media_only and self.static_only:
            raise CommandError("Can't use --media-only and --staticfiles-only together. Better not use anything...")
    ...
    def sync_s3(self):
        """Walk the media/staticfiles directories and syncs files to S3"""
